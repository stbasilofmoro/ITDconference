...
export default function CarbonSort({ ctx }: { ctx: GameContext }) {
  ...
  return <>
    ...
    <PhonePortal><PhonePanel gestures={{ hint: "Swipe left/right to move. Tap or swipe up to rotate. Swipe down to drop.", tap: () => act("rotate"), swipe: (d) => act(d === "up" ? "rotate" : d === "down" ? "drop" : d) }} summary={<span>Next: {run.next.map((m) => MATERIAL_NAMES[m]).join(" + ")}</span>} title="Carbon Sort" status={`Round ${run.round + 1} / 3 · ${run.score} points · ${left} stock left`} modal={modal} exit={ctx.exit} result={run.phase === 'won' || run.phase === 'lost' ? { id: scoreId.current, game: 'carbon-sort', score: run.score, detail: `${run.recovered} materials recovered / ${run.targetsCleared} stock cleared` } : undefined}>
