...
export default function KilnKeeper({ ctx }: { ctx: GameContext }) {
  ...
  return <>
    <KilnScene run={run} />
    <PhonePortal><PhonePanel gestures={{ hint: "Drag across the screen to set feed: left = off, right = full. Tap to add wood.", tap: select, drag: change }} summary={<span>{Math.round(run.temperature)} C / Feed {Math.round(run.feed * 100)}% / {outside ? `Too ${outside}! ${Math.max(0, GRACE_SECONDS - debt).toFixed(1)}s to recover` : `${run.trend.toFixed(1)} C/s`}</span>} title="Kiln Keeper" status={`${Math.max(0, Math.ceil(RUN_SECONDS - run.elapsed))}s left · Target ${RANGE}`} modal={modal} exit={ctx.exit} result={result ? { id: scoreId.current, game: 'kiln-keeper', score: Math.floor(run.inBand * 100) + (won ? 10000 : 0), detail: `${run.inBand.toFixed(1)} seconds in range / ${won ? 'Batch complete' : cold ? 'Cold batch' : 'Overheated'}` } : undefined}>
